...
SDLWindow::SDLWindow()
  : m_width(640)
  , m_height(400) {
  logger.log(io::LOG_INFO, "initializing sdl");
  
  if (SDL_Init(SDL_INIT_EVERYTHING) != 0) {
    logger.log(io::LOG_INFO) << "Unable to initialize SDL: " << SDL_GetError() << std::endl;
    throw std::runtime_error(SDL_GetError());
  }
}

void SDLWindow::open() {
  logger.log(io::LOG_INFO, "opening window");
  ...
  if (m_window == NULL) {
    logger.log(io::LOG_INFO, "failed initializing the window.");
    throw std::runtime_error(SDL_GetError());
  }
  
  m_context = SDL_GL_CreateContext(m_window.get());
  if (m_context == NULL) {
    logger.log(io::LOG_INFO, "failed initializing the context.");
    throw std::runtime_error(SDL_GetError());
  }
  
  if (!(IMG_Init(IMG_INIT_PNG) & IMG_INIT_PNG)) {
    logger.log(io::LOG_INFO, "failed initializing sdl image.");
    throw std::runtime_error(SDL_GetError());
  }
  
  if (TTF_Init() != 0) {
    logger.log(io::LOG_INFO, "failed initializing sdl ttf");
    throw std::runtime_error(SDL_GetError());
  }
...
SDLWindow::~SDLWindow() {
  logger.log(io::LOG_INFO, "closing window");
...
void SDLWindow::handleEvent(SDL_Event event) {
  ...
  if (event.type == SDL_WINDOWEVENT) {
    if (event.window.event == SDL_WINDOWEVENT_RESIZED) {
      logger.log(io::LOG_INFO, "resized window");
      if (m_initializedgl) {
